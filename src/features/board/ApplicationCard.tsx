  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 10, color: 'var(--c-9a978f)', fontWeight: 500, letterSpacing: '0.02em' }}>{id}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, minWidth: 0, marginTop: 1 }}>
        {contacts.length > 0 && (
          <>
            ...
            <div style={{ fontSize: 10.5, color: 'var(--c-5f5c56)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>
              {contacts[0].name}
            </div>
            {contacts.length > 1 && (
              <div style={{ fontSize: 10.5, color: 'var(--c-9a978f)', flexShrink: 0 }}>+{contacts.length - 1}</div>
            )}
          </>
        )}
        {!run && !interview && (
      ...
      {interview && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 1, minWidth: 0 }}>
          <div style={{
            width: 30, flexShrink: 0, borderRadius: 6, overflow: 'hidden', textAlign: 'center',
            border: '1px solid color-mix(in srgb, ' + col.accent + ' 26%, var(--c-fff))', background: 'var(--c-fff)',
          }}>
            <div style={{ background: col.accent, color: 'var(--c-fff)', fontSize: 7.5, fontWeight: 700, letterSpacing: '0.06em', padding: '1px 0' }}>
              {interview.month}
            </div>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--c-1b1a17)', lineHeight: 1.25, padding: '1px 0 2px' }}>{interview.day}</div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--c-1b1a17)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {interview.time}
            </div>
            <div style={{ fontSize: 10, color: 'var(--c-8b8880)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {interview.meta}
            </div>
          </div>
        </div>
      ...
      {run && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0, background: 'var(--c-fff)', borderRadius: 6, padding: '5px 7px' }}>
          <Spinner />
          <div style={{
            fontSize: 9.5, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
            backgroundImage: 'linear-gradient(90deg,var(--c-a5a29a) 0%,var(--c-a5a29a) 28%,var(--c-1b1a17) 46%,var(--c-a5a29a) 64%,var(--c-a5a29a) 100%)',
          ...
          <div style={{
            marginLeft: 'auto', fontSize: 9.5, color: 'var(--c-8b8880)', flexShrink: 0,
