    ...
    return {
        ...
        createApplication(input: {
            ...
            return tx(() => {
                ...
                db.prepare(
                    "UPDATE meta SET value = ? WHERE key = 'next_bew_num'",
                ).run(String(num + 1));
                const id = "BEW-" + num;

                const company = findOrCreateCompany(
                    input.company || "Unbekanntes Unternehmen",
                );
                db.prepare(
        ...
        relinkCompany(
            id: string,
            name: string,
        ): { application: ApplicationRow; company: CompanyRow } {
            return tx(() => {
                const company = findOrCreateCompany(name);
                db.prepare(
                    "UPDATE applications SET company_id = ?, updated_at = ? WHERE id = ?",
                ).run(company.id, nowISO(), id);
        ...
        deleteFact(applicationId: string, label: string): void {
            tx(() => {
                db.prepare(
                    "DELETE FROM facts WHERE application_id = ? AND label = ?",
                ).run(applicationId, label);
                touchApplication(applicationId);
            });
        },
       ...
       Rows arriving with an id keep it (and their notes); the rest are new. */
        setRounds(
            ...
            return tx(() => {
                ...
                for (const rid of existing)
                    if (!keep.includes(rid)) del.run(rid);

                /* Two passes so UNIQUE(application_id, position) never collides mid-update. */
                const park = db.prepare(
                    "UPDATE rounds SET position = position + 10000 WHERE application_id = ?",
                );
           ...
           VALUES (?,?,?,?,?,?,?,?,?)`,
                ...
                const clearPeople = db.prepare(
                    "DELETE FROM round_people WHERE round_id = ?",
                );
        ...
        setApplicationPeople(
            ...
            return tx(() => {
                db.prepare(
                    "DELETE FROM application_people WHERE application_id = ? AND kind = ?",
                ).run(applicationId, kind);
        ...
        setFollowupDue(followupId: number, dueAt: string): FollowupRow {
            return tx(() => {
                db.prepare("UPDATE followups SET due_at = ? WHERE id = ?").run(
                    dueAt,
                    followupId,
                );
