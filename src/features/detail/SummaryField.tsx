...
export function SummaryField({ cardId, summary, locked }: { cardId: string; summary: string; locked: boolean }) {
  ...
  if (st.editing === 'summary') {
    return (
      <textarea
        ...
        onKeyDown={(e) => {
          if (e.key === 'Escape') { e.stopPropagation(); cancelEditRef.current = true; e.currentTarget.blur(); }
        ...
        style={{
          ...
          outline: 'none', resize: 'none', fieldSizing: 'content', minHeight: 0, width: '100%', boxSizing: 'border-box',
  ...
  return (
    <div
      ...
      onClick={() => { if (!locked) set({ editing: 'summary', editDraft: summary, dropdown: null }); }}
      style={{
        fontSize: 12.5, color: 'var(--c-5f5c56)', lineHeight: 1.6, textWrap: 'pretty',
        padding: '8px 10px', cursor: locked ? 'not-allowed' : 'text',
      }}
    >
      {summary}
    </div>
