...
export function ModalShell({
  onClose, header, footer, children,
}: {
  onClose: () => void;
  header: ReactNode;
  footer: ReactNode;
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 }}>
      ...
      <div style={{
        position: 'relative', width: 660, maxHeight: 'calc(100% - 48px)',
        display: 'flex', flexDirection: 'column',
        background: 'var(--c-fff)', borderRadius: 14, boxShadow: '0 24px 70px var(--s-2)',
      }}>
        <div className="no-scrollbar" style={{ padding: '20px 24px 16px', display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
...
export function SubmitButton({ label, enabled = true, onClick }: {
  label: string;
  enabled?: boolean;
  onClick: () => void;
}) {
  return (
    <div
      ...
      style={{
        ...
        fontSize: 13.5, fontWeight: 600, cursor: enabled ? 'pointer' : 'not-allowed',
      }}
    >
      <span>{label}</span>
      <span style={{ background: 'var(--s-6)', borderRadius: 4, padding: '1px 5px', fontSize: 12, fontWeight: 500 }}>⌘</span>
      <span style={{ background: 'var(--s-6)', borderRadius: 4, padding: '1px 5px', fontSize: 12, fontWeight: 500 }}>↵</span>
    </div>
  );
}
